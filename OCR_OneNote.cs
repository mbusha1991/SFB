 ...
 * Date: 31/07/2017
 * Description: Interaction with One Note to Copy text from image
*/
...
namespace SFBTesting.LibraryFunctions
{
        ...
        : IDisposable
    {
        ...
        public OCR()
        {
             var app = this.Try(() => new Application(), e => new Exception("Error initializing OneNote", e));
           
            this._page = this.Try(() => new OneNotePage(app), e => new Exception("Error initializing page.", e));
        }


        public string Recognize(Image image)
        {
            return this.Try(() => this.RecognizeIntern(image), e => new Exception("Error during recognition", e));
        }
